...
from models.neural_nets import ConvModel, NM_AidedConvModel
...
SAVE_PATH = 'logs/'
OUTPUT_FILE = SAVE_PATH + "NM_Conscious_Model1.csv"
...
def main(run, dataset_dict, print_messages=False):
    ...
    prefix = "NM_Conscious_Model1"
    model = ConvModel(num_outputs=60)
    ...
    aided_model = NM_AidedConvModel(num_outputs=60)
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        with Bar('{} Run {:03d} Epoch {:04}'.format(prefix, run, epoch + 1), max=BATCHES_PER_EPOCH) as bar:
            for i in range(BATCHES_PER_EPOCH):
                ...
                aid = hebb_aid.call(raw[hebb_number-1], one_hot=True)
...
if __name__ == "__main__":
    print('Conscious_Model1 training script')
